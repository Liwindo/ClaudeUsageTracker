...
These exercise the pure logic that the bug lived in — the authoritative
banner-free frame (`_base_*`), its bottom-anchored display geometry, and the
persistence path — without a live Tk window. Tk geometry round-trips through
...
def test_save_persists_banner_free_base_even_while_banner_shown(tmp_path, monkeypatch):
    ...
    w._base_x, w._base_y, w._base_w, w._base_h = 600, 400, 256, 186
    w._extra_for_banner = 27                    # banner up — must NOT affect the save
    w._minimized = False
    w._save_position()
